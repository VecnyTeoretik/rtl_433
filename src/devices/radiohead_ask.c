...
static int sensible_living_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
   ...
   /*  house_id = rh_payload[1];
    ...
    data = data_make(
			...
			"freq",             "frekvence",         DATA_DOUBLE,     (double)decoder->centerfreq_hz,
            ...
            "rssi",     	    "rssi db",     	    DATA_DOUBLE,     (double)decoder->rssi_db,
			"snr",     	    "snr db",     	    DATA_DOUBLE,     (double)decoder->snr_db,
			"noise",         "noise db",         DATA_DOUBLE,     (double)decoder->noise_db,
            "mic",           	"Integrity",        DATA_STRING,  "CRC",
